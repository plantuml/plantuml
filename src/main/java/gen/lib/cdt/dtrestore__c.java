...
try {
	...
	{	//dt->openiconic->here = ((Dtlink_t*)0);
UNSUPPORTED("4xawc48hce5sov89n8h4j7xw0"); // 		if(type) /* restoring a flattened dictionary */
UNSUPPORTED("3yrjmgus9a9415ocrrtj1e733"); // 		{	for(ends = (s = dt->openiconic->hh._htab) + dt->openiconic->ntab; s < ends; ++s)
...
UNSUPPORTED("efiynrcqevfta9kp0bfbujmn5"); // 		{	dt->openiconic->size = 0;
...
UNSUPPORTED("6eq5kf0bj692bokt0bixy1ixh"); // 		}
	...
	{	if((dt.getPtr("data").getInt("type")&(0000004|0000010))!=0)
		...
		else /*if(dt->openiconic->type&(DT_LIST|DT_STACK|DT_QUEUE))*/
{UNSUPPORTED("79wu0m7edfdq02msgoqkzb32"); // 		{	dt->openiconic->here = ((Dtlink_t*)0);
UNSUPPORTED("c8q10nh3f6o4rjxtacmbgjxqj"); // 			dt->openiconic->hh._head = list;
		}
		if(N(type))
