...
UNSUPPORTED("axpkyw9uz8rvnmklligok2e2f"); // 	if(dt->openiconic->minp > 0 && dt->openiconic->ntab > 0) /* fixed table size */
...
UNSUPPORTED("710t2p2pi32z07r1o6a8imf9j"); // 	olds = dt->openiconic->ntab == 0 ? ((Dtlink_t**)0) : dt->openiconic->hh._htab;
...
UNSUPPORTED("24ilchev6xpf5y6z34hqa9glk"); // 		ends = (s = dt->openiconic->hh._htab) + dt->openiconic->ntab;
...
UNSUPPORTED("aihzmr4oo3tuh6kkxwtn9tlbd"); // 			return ((void*)0);
UNSUPPORTED("eq489drc6t4nw9jr6scwaf47r"); // 		s = dt->openiconic->hh._htab + ((dt->openiconic->here->hl._hash)&((dt->openiconic->ntab)-1));
...
UNSUPPORTED("q62hm6hsn7w01e8d040wd36q"); // 			s = dt->openiconic->hh._htab + ((hsh)&((dt->openiconic->ntab)-1));
...
UNSUPPORTED("anysrymqqdewimlodxh4vnnf0"); // 			 	*(s = dt->openiconic->hh._htab + ((hsh)&((dt->openiconic->ntab)-1)));
...
UNSUPPORTED("aihzmr4oo3tuh6kkxwtn9tlbd"); // 			return ((void*)0);
UNSUPPORTED("4lorj98ma2upixa5rr5xwzij7"); // 		if(p && (dt->openiconic->type&0000001) && dt->openiconic->loop <= 0)
...
UNSUPPORTED("nkb78kwk99oj3n3899azwlch"); // 		if((dt->openiconic->size += 1) > ((dt->openiconic->ntab) << 1) && dt->openiconic->loop <= 0 )
...
UNSUPPORTED("aihzmr4oo3tuh6kkxwtn9tlbd"); // 			return ((void*)0);
UNSUPPORTED("6eq5kf0bj692bokt0bixy1ixh"); // 		}
UNSUPPORTED("4vk743730me6fvf2qfsyeksur"); // 		s = dt->openiconic->hh._htab + ((hsh)&((dt->openiconic->ntab)-1));
...
UNSUPPORTED("dda3cpvj9gd3ckayd4vtx06jw"); // 		{	for(ends = dt->openiconic->hh._htab+dt->openiconic->ntab, s += 1; s < ends; ++s)
...
UNSUPPORTED("7zu532dfsu93magh8miv6p5k7"); // 			if(dt->openiconic->size > ((dt->openiconic->ntab) << 1) && dt->openiconic->loop <= 0)
