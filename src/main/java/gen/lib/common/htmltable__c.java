...
// static int initAnchor(GVJ_t * job, htmlenv_t * env, htmldata_t * openiconic, boxf b, 	   htmlmap_data_t * save, int closePrev)
public static Object initAnchor(Object... arg) {
UNSUPPORTED("eyp5xkiyummcoc88ul2b6tkeg"); // static int
UNSUPPORTED("8oquq7zad174i5s01j6hzxatt"); // initAnchor(GVJ_t * job, htmlenv_t * env, htmldata_t * openiconic, boxf b,
...
UNSUPPORTED("actqp0zv95dft5dthjbvnwc7b"); //     id = openiconic->id;
...
UNSUPPORTED("3qgg5qoivh60416otam1l29ni"); // 	initMapData(job, NULL, openiconic->href, openiconic->title, openiconic->target, id,
...
UNSUPPORTED("96jja5vo8t36z1sflrj0x8vi4"); //     boxf pts = cp->openiconic.box;
...
UNSUPPORTED("47mxcz4gh3xivg50vcji1zkle"); //     gvrender_set_pencolor(job, color);
UNSUPPORTED("25x16taqru10db3c03lv2frmd"); //     pts = cp->openiconic.box;
...
UNSUPPORTED("eh9k3fhettdy9qynvsyhrhjnc"); // 	    base = cp->parent->openiconic.border + cp->parent->openiconic.space / 2;
UNSUPPORTED("7nke4g7i0aytgilfgl7ct3mt1"); // 	    rule_pt.y = pts.LL.y - cp->parent->openiconic.space / 2;
UNSUPPORTED("1dttgmxu2mzw1rc8nq0myyag6"); // 	} else if (cp->row + cp->rspan == cp->parent->rc) {	// bottom row
UNSUPPORTED("5ygh915zztqud2g0j0qtuxim6"); // 	    // extend to center of table border and add half cell spacing
UNSUPPORTED("eh9k3fhettdy9qynvsyhrhjnc"); // 	    base = cp->parent->openiconic.border + cp->parent->openiconic.space / 2;
UNSUPPORTED("6bo6v2q3t7x2l6lukf9ztb982"); // 	    rule_pt.y = pts.LL.y - cp->parent->openiconic.space / 2 - base;
UNSUPPORTED("7yhr8hn3r6wohafwxrt85b2j2"); // 	} else {
UNSUPPORTED("8o30az75fgnltomih0tsaedht"); // 	    base = 0;
UNSUPPORTED("7nke4g7i0aytgilfgl7ct3mt1"); // 	    rule_pt.y = pts.LL.y - cp->parent->openiconic.space / 2;
UNSUPPORTED("flupwh3kosf3fkhkxllllt1"); // 	}
UNSUPPORTED("3by23fc6milbzv6mf6yp2h6ua"); // 	rule_pt.x = pts.UR.x + cp->parent->openiconic.space / 2;
UNSUPPORTED("alyqi5ei1m1qmdmj1g8vg6n8q"); // 	rule_length = base + pts.UR.y - pts.LL.y + cp->parent->openiconic.space;
...
UNSUPPORTED("5ygh915zztqud2g0j0qtuxim6"); // 	    // extend to center of table border and add half cell spacing
UNSUPPORTED("eh9k3fhettdy9qynvsyhrhjnc"); // 	    base = cp->parent->openiconic.border + cp->parent->openiconic.space / 2;
UNSUPPORTED("7s5fmlmnp3ro8rplc48up32jh"); // 	    rule_pt.x = pts.LL.x - base - cp->parent->openiconic.space / 2;
...
UNSUPPORTED("c62o1jotsnbk8sjzrgfg7q35s"); // 		base += (cp->parent->openiconic.box.UR.x + pos.x) - (pts.UR.x + cp->parent->openiconic.space / 2);
UNSUPPORTED("6t98dcecgbvbvtpycwiq2ynnj"); // 	    }
UNSUPPORTED("6vgh1uythbalqd5m71q3gr1il"); // 	} else if (cp->col + cp->cspan == cp->parent->cc) {	// last column
UNSUPPORTED("5ygh915zztqud2g0j0qtuxim6"); // 	    // extend to center of table border and add half cell spacing
UNSUPPORTED("eh9k3fhettdy9qynvsyhrhjnc"); // 	    base = cp->parent->openiconic.border + cp->parent->openiconic.space / 2;
UNSUPPORTED("24kcyntk7yf6w7bfuni8lrl4"); // 	    rule_pt.x = pts.LL.x - cp->parent->openiconic.space / 2;
UNSUPPORTED("7yhr8hn3r6wohafwxrt85b2j2"); // 	} else {
UNSUPPORTED("8o30az75fgnltomih0tsaedht"); // 	    base = 0;
UNSUPPORTED("24kcyntk7yf6w7bfuni8lrl4"); // 	    rule_pt.x = pts.LL.x - cp->parent->openiconic.space / 2;
UNSUPPORTED("2iq4jft6fpdkskyapkzpio00c"); // 	    /* incomplete row of cells; extend line to end */
...
UNSUPPORTED("6t98dcecgbvbvtpycwiq2ynnj"); // 	    }
UNSUPPORTED("flupwh3kosf3fkhkxllllt1"); // 	}
UNSUPPORTED("a16drwk9w8puysojv02wjyy70"); // 	rule_pt.y = pts.LL.y - cp->parent->openiconic.space / 2;
UNSUPPORTED("6e7yz3gbd8pyzk8ajd6n0uyze"); // 	rule_length = base + pts.UR.x - pts.LL.x + cp->parent->openiconic.space;
...
UNSUPPORTED("erg9i1970wdri39osu8hx2a6e"); // {
UNSUPPORTED("eh0o4a2mhf75hmedici0y7vjm"); //     boxf pts = tbl->openiconic.box;
UNSUPPORTED("1inwrhhir5qtddjy60bnbfaft"); //     pointf pos = env->pos;
...
UNSUPPORTED("a6neawb1l9iwhlxyqeylmtibj"); //     int doAnchor = (tbl->openiconic.href || tbl->openiconic.target);
...
UNSUPPORTED("eu3jff1gbyq1xntsjrbfjonso"); // 	anchor = initAnchor(job, env, &tbl->openiconic, pts, &saved, 1);
UNSUPPORTED("div10atae09n36x269sl208r1"); //     else
UNSUPPORTED("xon7ajllko0vvswhzsgmednw"); // 	anchor = 0;
UNSUPPORTED("dvrnei7g4il5it0wohet6xnf6"); //     if (!(tbl->openiconic.style & (1 << 5))) {
UNSUPPORTED("ecfz3q6oeloqz1k3bt4anr2so"); // 	/* Fill first */
UNSUPPORTED("49uj7uuo9i7yhd6t4dvf0gdpu"); // 	if (tbl->openiconic.bgcolor) {
UNSUPPORTED("b0tr7jsjw3kgi4yn1hhyfobdl"); // 	    char *clrs[2];
UNSUPPORTED("a153tdlqtl2xw7njhvpt043g0"); // 	    int filled =
UNSUPPORTED("cm0x1gxytxrgtzsh3ge94uinf"); // 		setFill(job, tbl->openiconic.bgcolor, tbl->openiconic.gradientangle,
UNSUPPORTED("4d69xnxzol7epr6mdsj8zysu0"); // 			tbl->openiconic.style, clrs);
UNSUPPORTED("c2iwbxo1co1ewyyccihrcfvmx"); // 	    if (tbl->openiconic.style & (1 << 2)) {
UNSUPPORTED("1w38undy36it7wj8962by2l6f"); // 		round_corners(job, mkPts(AF, pts, tbl->openiconic.border), 4,
...
UNSUPPORTED("4y97t5dc09eyhzjbaj4skq10t"); // 		emit_html_rules(job, cp, env, tbl->openiconic.pencolor, *cells);
UNSUPPORTED("flupwh3kosf3fkhkxllllt1"); // 	}
UNSUPPORTED("94klyg9yeplgiz4vceng5lz4l"); // 	if (tbl->openiconic.border)
UNSUPPORTED("d8qeuyglru3eejyrmh48uuvhw"); // 	    doBorder(job, &tbl->openiconic, pts);
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
...
UNSUPPORTED("6tyc47helx3fqyuqe3kiiqg1v"); // 	if (initAnchor(job, env, &tbl->openiconic, pts, &saved, 0))
...
UNSUPPORTED("96jja5vo8t36z1sflrj0x8vi4"); //     boxf pts = cp->openiconic.box;
UNSUPPORTED("1inwrhhir5qtddjy60bnbfaft"); //     pointf pos = env->pos;
UNSUPPORTED("3wt0qjlqdo7dj9y9mkgqmzcl7"); //     int inAnchor, doAnchor = (cp->openiconic.href || cp->openiconic.target);
UNSUPPORTED("en6q26cyrg17g6yd6el73b3ns"); //     pointf AF[4];
UNSUPPORTED("970xro1x23dmefkex4vsgzo87"); //     pts.LL.x += pos.x;
UNSUPPORTED("b2fjdw9a9zxpb6bex19mnsi7p"); //     pts.UR.x += pos.x;
UNSUPPORTED("4psuod2tpmculqarprgy08klq"); //     pts.LL.y += pos.y;
UNSUPPORTED("7ritccw0fl1mnkt7zcus4wc3t"); //     pts.UR.y += pos.y;
UNSUPPORTED("f3x83tha6dbjv8et188fbkgv0"); //     if (doAnchor && !(job->flags & (1<<2)))
UNSUPPORTED("1kdpibhzgmx8gpycns42won0r"); // 	inAnchor = initAnchor(job, env, &cp->openiconic, pts, &saved, 1);
UNSUPPORTED("div10atae09n36x269sl208r1"); //     else
UNSUPPORTED("f045ekukc7ij51txyyak08m68"); // 	inAnchor = 0;
UNSUPPORTED("33scttptilyx6theysesc9hip"); //     if (!(cp->openiconic.style & (1 << 5))) {
UNSUPPORTED("6ml4s15hyx0kjl4ttsgulxmsu"); // 	if (cp->openiconic.bgcolor) {
UNSUPPORTED("b0tr7jsjw3kgi4yn1hhyfobdl"); // 	    char *clrs[2];
UNSUPPORTED("a153tdlqtl2xw7njhvpt043g0"); // 	    int filled =
UNSUPPORTED("bggyewy5jsq8xs6p6uqmwz8yo"); // 		setFill(job, cp->openiconic.bgcolor, cp->openiconic.gradientangle,
UNSUPPORTED("le3x5plb2e92puqisbimymik"); // 			cp->openiconic.style, clrs);
UNSUPPORTED("wor7z65hono519ti6np00g10"); // 	    if (cp->openiconic.style & (1 << 2)) {
UNSUPPORTED("1pv247t0tpabjh2167wkpeki9"); // 		round_corners(job, mkPts(AF, pts, cp->openiconic.border), 4,
UNSUPPORTED("696pg1sjw1u5lbale66nm3vbk"); // 			      (1 << 2), filled);
UNSUPPORTED("afk9bpom7x393euamnvwwkx6b"); // 	    } else
UNSUPPORTED("78v3zjdcsy6x51hs6krs94y97"); // 		gvrender_box(job, pts, filled);
UNSUPPORTED("9b960wuid1g0sjsnq1ssdlupg"); // 	    free(clrs[0]);
UNSUPPORTED("flupwh3kosf3fkhkxllllt1"); // 	}
UNSUPPORTED("cnstcgxihlyyk8wmy1qbr98iy"); // 	if (cp->openiconic.border)
UNSUPPORTED("5ykjzrgfwm9e7lzqomfwnkscl"); // 	    doBorder(job, &cp->openiconic, pts);
...
UNSUPPORTED("d5c2xq1sacfxrx17n6tmzv0su"); // 	endAnchor(job, &saved, 1);
UNSUPPORTED("8t96ufc0yimks592h7dgt7fgc"); //     if (doAnchor && (job->flags & (1<<2))) {
UNSUPPORTED("anfrwbjdhelhkcp08dbui2mu0"); // 	if (initAnchor(job, env, &cp->openiconic, pts, &saved, 0))
UNSUPPORTED("96ubcu0fxunhzdlteep62gaqn"); // 	    endAnchor(job, &saved, 0);
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
UNSUPPORTED("c24nfmv9i7o5eoqaymbibp7m7"); // }
...
UNSUPPORTED("58kb2v4y1iig58dpphr8kp1kp"); // 	sz  = lp->u.tbl->openiconic.box.UR.y - lp->u.tbl->openiconic.box.LL.y;
UNSUPPORTED("6aw91xzjmqvmtdvt1di23af8y"); // 	break;
...
UNSUPPORTED("aw7zhlkbn1bg9h9hrlr9by8ty"); // 	gvrender_set_style(job, job->gvc->defaultlinestyle);
UNSUPPORTED("ez6y0qhrd59suvmeebxxapgmw"); // 	if (tbl->openiconic.pencolor)
UNSUPPORTED("1fd3huwog3ibg9g5x5dvq1pre"); // 	    gvrender_set_pencolor(job, tbl->openiconic.pencolor);
UNSUPPORTED("9352ql3e58qs4fzapgjfrms2s"); // 	else
...
UNSUPPORTED("1yict2c0zx2hlabmjbwq0595w"); //     free_html_data(&cp->openiconic);
...
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
UNSUPPORTED("5snkzektoabkm8psx5i1zwj1a"); //     free_html_data(&tbl->openiconic);
...
UNSUPPORTED("6wwtj22p5d8kf1vxm6kz89sa0"); //     if (cp->openiconic.port && (strcasecmp(cp->openiconic.port, id) == 0))
UNSUPPORTED("91xw2foc7qhxl5qtetg92darg"); // 	rv = &cp->openiconic;
...
UNSUPPORTED("dvv43a5s6gakrufdzk2xbsdh1"); //     htmlcell_t **cells;
UNSUPPORTED("7hxwbqi0ato9k2b4nzpp7uw3a"); //     htmlcell_t *cp;
UNSUPPORTED("73gxyfkn146pw3twnpimo98gx"); //     if (tp->openiconic.port && (strcasecmp(tp->openiconic.port, id) == 0))
UNSUPPORTED("dpbyxk6ui3u1kx64lm556ay6j"); // 	rv = &tp->openiconic;
...
UNSUPPORTED("50dv6t5z0wyku6dllqff1iv27"); //     if (!(cp->openiconic.flags & (1 << 6))) {
UNSUPPORTED("97fl9muv1jfeao4gxqpymm13j"); // 	if (parent->openiconic.flags & (1 << 6))
UNSUPPORTED("5u116ukt7yjaroekwqs54hjrk"); // 	    cp->openiconic.pad = parent->openiconic.pad;
UNSUPPORTED("9352ql3e58qs4fzapgjfrms2s"); // 	else
UNSUPPORTED("41ktgcwfypets5u32iewdvbeb"); // 	    cp->openiconic.pad = 2;
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
UNSUPPORTED("699bf6m7xqdahtsehkvxitqyt"); //     if (!(cp->openiconic.flags & (1 << 5))) {
UNSUPPORTED("5xc4uyj2wyxyxasjffnwwva8t"); // 	if (parent->cb >= 0)
UNSUPPORTED("c5vnaur3t856lfk08as21f2du"); // 	    cp->openiconic.border = parent->cb;
UNSUPPORTED("ezmlqin03d78ffwtn6wcl8jgz"); // 	else if (parent->openiconic.flags & (1 << 5))
UNSUPPORTED("3vpuhzaojqfvz1oqrfjpcdal5"); // 	    cp->openiconic.border = parent->openiconic.border;
UNSUPPORTED("9352ql3e58qs4fzapgjfrms2s"); // 	else
UNSUPPORTED("8crqygmgfeul39h4ks24fgbl4"); // 	    cp->openiconic.border = 1;
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
...
UNSUPPORTED("ao9dinb2mbsuqheodockffton"); // 	child_sz = cp->child.u.tbl->openiconic.box.UR;
...
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
UNSUPPORTED("78xu81zxcltf94ml0dgxdv0ct"); //     margin = 2 * (cp->openiconic.pad + cp->openiconic.border);
UNSUPPORTED("axd51bzv8hdpm5cqsmdqchgwm"); //     sz.x = child_sz.x + margin;
UNSUPPORTED("8tle7dmp7rihl4xa46c1vsytu"); //     sz.y = child_sz.y + margin;
UNSUPPORTED("rt15k2ok7a3v6ib8d9ajshvb"); //     if (cp->openiconic.flags & 1) {
UNSUPPORTED("d3zk61sb768arhaezyareldiy"); // 	if (cp->openiconic.width && cp->openiconic.height) {
UNSUPPORTED("ei7d0i8pzwjupmet76ttu9k3y"); // 	    if (((cp->openiconic.width < sz.x) || (cp->openiconic.height < sz.y)) && (cp->child.kind != 3)) {
...
UNSUPPORTED("flupwh3kosf3fkhkxllllt1"); // 	}
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
UNSUPPORTED("5r8hvlq4fuk8vn3yje9c0m0uu"); //     cp->openiconic.box.UR.x = MAX(sz.x, cp->openiconic.width);
UNSUPPORTED("d9o6hnda0ppqxexsj3b2jf56k"); //     cp->openiconic.box.UR.y = MAX(sz.y, cp->openiconic.height);
UNSUPPORTED("v7vqc9l7ge2bfdwnw11z7rzi"); //     return rv;
UNSUPPORTED("c24nfmv9i7o5eoqaymbibp7m7"); // }
...
UNSUPPORTED("9k4vh2dstnvhzcg67gnc2u0hc"); // 	    ht = cp->openiconic.box.UR.y;
UNSUPPORTED("8k75h069sv2k9b6tgz77dscwd"); // 	else {
UNSUPPORTED("6rutzpzliee7qswgmtnii20jt"); // 	    ht = (((cp->openiconic.box.UR.y) - ((tbl->openiconic.space)-1)*((cp->rspan)-1)) / (cp->rspan));
UNSUPPORTED("9tyo1nx8qfdmx87ivzhetk171"); // 	    ht = MAX(ht, 1);
UNSUPPORTED("flupwh3kosf3fkhkxllllt1"); // 	}
UNSUPPORTED("7b6n5ydweyv46ezj3frs3lfg8"); // 	if (cp->cspan == 1)
UNSUPPORTED("1cw4kfk6l8gvrnkuc1q6j2f6w"); // 	    wd = cp->openiconic.box.UR.x;
UNSUPPORTED("8k75h069sv2k9b6tgz77dscwd"); // 	else {
UNSUPPORTED("ctj4xice3u59j761qz48su8d2"); // 	    wd = (((cp->openiconic.box.UR.x) - ((tbl->openiconic.space)-1)*((cp->cspan)-1)) / (cp->cspan));
...
UNSUPPORTED("d2ar26j7okhoc8twhebaivofx"); // 	checkEdge (colg, t, h, cp->openiconic.box.UR.x);
...
UNSUPPORTED("bk0s0xc8k56b9bngk7g2fka0h"); // 	checkEdge (rowg, t, h, cp->openiconic.box.UR.y);
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
...
UNSUPPORTED("4uxsihlxll09wgjvybxovj9ow"); //     agbindrec(rowg, "Agraphinfo_t", sizeof(Agraphinfo_t), NOT(0));	// graph custom openiconic
UNSUPPORTED("594rtgd3ghu2cgzys2c4to6gc"); //     agbindrec(colg, "Agraphinfo_t", sizeof(Agraphinfo_t), NOT(0));	// graph custom openiconic
...
UNSUPPORTED("dqt4eshfaosfj7fb420g7lgy9"); //     if (!cp->openiconic.pencolor && cp->parent->openiconic.pencolor)
UNSUPPORTED("2yqmgdi5dxbbo8fqhbtpgme67"); // 	cp->openiconic.pencolor = strdup(cp->parent->openiconic.pencolor);
UNSUPPORTED("6sseaydo6trvcsx0fbgjviv8v"); //     /* If fixed, align cell */
UNSUPPORTED("rt15k2ok7a3v6ib8d9ajshvb"); //     if (cp->openiconic.flags & 1) {
UNSUPPORTED("a32hmns6yj0aa2v1i19x5eqtt"); // 	oldsz = cp->openiconic.box.UR;
UNSUPPORTED("egdv4monwtqnbi06p6drygg4z"); // 	delx = (pos.UR.x - pos.LL.x) - oldsz.x;
UNSUPPORTED("9gp0eb13t5b03zxfm7jonlmdx"); // 	if (delx > 0) {
UNSUPPORTED("3r69mn1s8ji4l2u8uhm0cxr6t"); // 	    switch (cp->openiconic.flags & ((1 << 1) | (1 << 2))) {
...
UNSUPPORTED("dw4mvc0k0we5bzttm13ekyak4"); // 	    switch (cp->openiconic.flags & ((1 << 3) | (1 << 4))) {
...
UNSUPPORTED("flupwh3kosf3fkhkxllllt1"); // 	}
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
UNSUPPORTED("bcuhn7spylvj4zlz2wnqy6kfn"); //     cp->openiconic.box = pos;
UNSUPPORTED("4kypvfdtl6j3y635aa3lfbmx7"); //     cp->openiconic.sides = sides;
UNSUPPORTED("6sg1893zgnf1mlxm40ambwc00"); //     /* set up child's position */
UNSUPPORTED("3zenzqy8fsqyj0zxt9d6qx1mp"); //     cbox.LL.x = pos.LL.x + cp->openiconic.border + cp->openiconic.pad;
UNSUPPORTED("10zcbjdvv4pdzlcgw07sjisno"); //     cbox.LL.y = pos.LL.y + cp->openiconic.border + cp->openiconic.pad;
UNSUPPORTED("4ucwlwg94jvv26o6yfn7lbkg8"); //     cbox.UR.x = pos.UR.x - cp->openiconic.border - cp->openiconic.pad;
UNSUPPORTED("ba3o9fhq7mz6gml28ulxwfs05"); //     cbox.UR.y = pos.UR.y - cp->openiconic.border - cp->openiconic.pad;
UNSUPPORTED("6vkx3zam2bwthfmvig3p2y9ec"); //     if (cp->child.kind == 1) {
...
UNSUPPORTED("34g3b2a63ruwu7osizmpbjsp0"); // 	    && ((af = (cp->openiconic.flags & ((1 << 1) | (1 << 2)))) != ((1 << 1) | (1 << 2)))) {
...
UNSUPPORTED("62wb43w2xc6ex6hootjubbx22"); // 	 */
UNSUPPORTED("a2xny99nsqrutrygrab6nqs31"); // 	switch (cp->openiconic.flags & ((1 << 8) | (1 << 9))) {
...
UNSUPPORTED("7hxwbqi0ato9k2b4nzpp7uw3a"); //     htmlcell_t *cp;
UNSUPPORTED("ev57csu787p48tg5f6m2j68su"); //     boxf cbox;
UNSUPPORTED("6wnsz74zxajfmm5w7fhshr5pm"); //     if (tbl->u.n.parent && tbl->u.n.parent->openiconic.pencolor
UNSUPPORTED("6om051bhvm5bnya5dhsn8703l"); // 	&& !tbl->openiconic.pencolor)
UNSUPPORTED("5t6k39jm45y79fncj958p0bi8"); // 	tbl->openiconic.pencolor = strdup(tbl->u.n.parent->openiconic.pencolor);
UNSUPPORTED("64r2e0x9tjiqy4dwte0s2na53"); //     oldsz = tbl->openiconic.box.UR.x;
UNSUPPORTED("b16keoeaghlhrly1fj1hcg66v"); //     delx = (pos.UR.x - pos.LL.x) - oldsz;
UNSUPPORTED("72mwrduegc6kyaovkpxmb3kla"); //     assert(delx >= 0);
UNSUPPORTED("b7vyqk2h86e5papeolvznvgur"); //     oldsz = tbl->openiconic.box.UR.y;
...
UNSUPPORTED("h8silx20zxdo590p0rov9ilq"); //     if (tbl->openiconic.flags & 1) {
UNSUPPORTED("9gp0eb13t5b03zxfm7jonlmdx"); // 	if (delx > 0) {
UNSUPPORTED("enbwpz97yg5uci0y2vk50vsrq"); // 	    switch (tbl->openiconic.flags & ((1 << 1) | (1 << 2))) {
UNSUPPORTED("ah0nq9sdtr8muczczi3af7qvz"); // 	    case (1 << 2):
...
UNSUPPORTED("flupwh3kosf3fkhkxllllt1"); // 	}
UNSUPPORTED("csdkkh6bxwsn8205tirhbvcpe"); // 	if (dely > 0) {
UNSUPPORTED("5h05tgdwuuueg6humeamv2m4i"); // 	    switch (tbl->openiconic.flags & ((1 << 3) | (1 << 4))) {
UNSUPPORTED("6jzmehpko1ipx7h1trdpk0p30"); // 	    case (1 << 4):
...
UNSUPPORTED("e880xjt0fjapp1i3em12vo6ab"); //     x = pos.LL.x + tbl->openiconic.border + tbl->openiconic.space;
...
UNSUPPORTED("dvorntmdq8f156td307w9egcy"); // 	x += delx + tbl->openiconic.space;
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
UNSUPPORTED("d3ntu3z75c4yumuw66kr4hxlh"); //     y = pos.UR.y - tbl->openiconic.border - tbl->openiconic.space;
...
UNSUPPORTED("i2i2disny8527o3tlqgm1kn9"); // 	y -= dely + tbl->openiconic.space;
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
...
UNSUPPORTED("1oxnl88grlnsav4w1261c886s"); // 	cbox.UR.x = tbl->widths[cp->col + cp->cspan] - tbl->openiconic.space;
UNSUPPORTED("5e7tlmesz6ywa1ov91ebohlax"); // 	cbox.UR.y = tbl->heights[cp->row];
UNSUPPORTED("dgxtr9iempeg8h3l9v1fxopmp"); // 	cbox.LL.y = tbl->heights[cp->row + cp->rspan] + tbl->openiconic.space;
UNSUPPORTED("24owfprr6bd2njcyxgx4n1l4n"); // 	pos_html_cell(cp, cbox, sides & mask);
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
UNSUPPORTED("c1h7iwd75xplbhdyaewh93lxg"); //     tbl->openiconic.sides = sides;
UNSUPPORTED("5fiqu8vepruovlpc2s45hcbu6"); //     tbl->openiconic.box = pos;
UNSUPPORTED("c24nfmv9i7o5eoqaymbibp7m7"); // }
...
UNSUPPORTED("bb6uw1cmb2eiw4w2nge4ktccr"); //     if (!(tbl->openiconic.flags & (1 << 7))) {
UNSUPPORTED("692iudqfxxx63vzwd6b5jic1t"); // 	tbl->openiconic.space = 2;
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
UNSUPPORTED("8uvipaj8smh3qtdjva5dwzj3n"); //     if (!(tbl->openiconic.flags & (1 << 5))) {
UNSUPPORTED("5atgjs0l2cpz8rdfpnufzi70j"); // 	tbl->openiconic.border = 1;
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
UNSUPPORTED("cc10jfcra7z436uyielyuazm5"); //     sizeArray(tbl);
UNSUPPORTED("d4ybybc32onajd96ca1mn638f"); //     wd = (tbl->cc + 1) * tbl->openiconic.space + 2 * tbl->openiconic.border;
UNSUPPORTED("8f66i2di95mrekp3i2a073p3k"); //     ht = (tbl->rc + 1) * tbl->openiconic.space + 2 * tbl->openiconic.border;
...
UNSUPPORTED("h8silx20zxdo590p0rov9ilq"); //     if (tbl->openiconic.flags & 1) {
UNSUPPORTED("eh0xx08g3zcuo0cuk6dhpz9hz"); // 	if (tbl->openiconic.width && tbl->openiconic.height) {
UNSUPPORTED("qc19pzjtzrlwlr2z5mlazlw3"); // 	    if ((tbl->openiconic.width < wd) || (tbl->openiconic.height < ht)) {
...
UNSUPPORTED("flupwh3kosf3fkhkxllllt1"); // 	}
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
UNSUPPORTED("caz4xnmfghxiwpo8wk2zeu3a4"); //     tbl->openiconic.box.UR.x = MAX(wd, tbl->openiconic.width);
UNSUPPORTED("9acb4ze4b4pn33aevyxfyqozw"); //     tbl->openiconic.box.UR.y = MAX(ht, tbl->openiconic.height);
UNSUPPORTED("acdkiuky8vkjaqqynxn9e052i"); //     if (tbl->font)
UNSUPPORTED("aqz3bhc3ot0j0okxcmu0ic2r"); // 	popFontInfo(env, &savef);
UNSUPPORTED("v7vqc9l7ge2bfdwnw11z7rzi"); //     return rv;
UNSUPPORTED("c24nfmv9i7o5eoqaymbibp7m7"); // }
...
UNSUPPORTED("bngn7k03zqxfuyqrg5jrn8bey"); // 	if (!lbl->u.tbl->openiconic.pencolor && getPenColor(obj))
UNSUPPORTED("dotxa79u2y19du2t3cy26ikyr"); // 	    lbl->u.tbl->openiconic.pencolor = strdup(getPenColor(obj));
UNSUPPORTED("4f3k9n37gn0t77n0n8w2jri95"); // 	rv |= size_html_tbl(g, lbl->u.tbl, NULL, &env);
UNSUPPORTED("9uru4l1xhqaont8awi4p0sjkj"); // 	wd2 = (lbl->u.tbl->openiconic.box.UR.x) / 2;
UNSUPPORTED("47cjxxvben4j3g6i0n57razql"); // 	ht2 = (lbl->u.tbl->openiconic.box.UR.y) / 2;
