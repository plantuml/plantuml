...
try {
    ...
    ctr = 0;
    if (false) SMETANA_TRACE("ns__c", "ns.init_rank: called, N_nodes=" + zz.N_nodes);
    for (v = GD_nlist(zz.G_ns); v!=null; v = ND_next(v)) {
	if (false) SMETANA_TRACE("ns__c", "ns.init_rank: node " + safeName(zz, v) + " priority=" + ND_priority(v)
    ...
    if (false) SMETANA_TRACE("ns__c", "ns.init_rank: done, ctr=" + ctr + " N_nodes=" + zz.N_nodes + (ctr != zz.N_nodes ? "  <<<< MISMATCH: some nodes never ranked, keep stale ND_rank!" : " (all nodes ranked OK)"));
    if (ctr != zz.N_nodes) {
	...
	for (v = GD_nlist(zz.G_ns); v!=null; v = ND_next(v))
	    if (ND_priority(v)!=0) 
		if (false) SMETANA_TRACE("ns__c", "ns.init_rank: unranked node " + safeName(zz, v) + " priority=" + ND_priority(v) + " stale rank=" + ND_rank(v));
    }
    free_queue(Q);
} finally {
...
try {
    ...
    if (false) SMETANA_TRACE("ns__c", "ns.rank2: init_graph returned feasible=" + feasible + " (0 means init_rank() will run next)");
    if (feasible == 0)
	init_rank(zz);
    if (maxiter <= 0) {
	freeTreeList (zz, g);
    ...
    if (false) SMETANA_TRACE("ns__c", "ns.rank2: feasible tree found, N_nodes=" + zz.N_nodes + " N_edges=" + zz.N_edges + " balance=" + balance + " maxiter=" + maxiter);
    // dumpNegativeSlackEdges(zz, "right after feasible_tree()");
    while ((e = leave_edge(zz))!=null) {
	f = enter_edge(zz, e);
	if (false) SMETANA_TRACE("ns__c", "ns.rank2 iter " + iter
	...
	update(zz, e, f);
	if (false) SMETANA_TRACE("ns__c", "ns.rank2 iter " + iter + ": after update, "
	...
	break;
    case 2:
    	if (false) SMETANA_TRACE("ns__c", "ns.rank2: main loop done after " + iter + " iterations, calling LR_balance");
	LR_balance(zz);
	break;
    default:
	scan_and_normalize(zz);
...
private static void dumpNegativeSlackEdges(Globals zz, String phase) {
	...
	for (int i = 0; ND_out(n).list.get_(i) != null; i++) {
	    ...
	    if (slack < 0) {
		bad++;
		if (false) SMETANA_TRACE("ns__c", "ns NEGATIVE SLACK " + phase + ": "
			+ safeName(zz, agtail(e)) + "(rank=" + ND_rank(agtail(e)) + ")"
			+ " -> " + safeName(zz, aghead(e)) + "(rank=" + ND_rank(aghead(e)) + ")"
    ...
    if (false) SMETANA_TRACE("ns__c", "ns dumpNegativeSlackEdges " + phase + ": " + bad + "/" + total + " edges have negative slack");
}
