...
public final class SmetanaDebug {
	...
	static public synchronized void SMETANA_TRACE(String fileName, String s) {
		try {
			if (traceWriter == null) {
				traceWriter = new java.io.PrintWriter(new java.io.FileWriter("smetana.txt", false));
			}
			traceWriter.println("[" + fileName + "]" + s.replace("\0", ""));
			traceWriter.flush();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	// agnameof() on such a node calls idprint -> Memory.fromIdentityHashCode(zz,
	// 0),
