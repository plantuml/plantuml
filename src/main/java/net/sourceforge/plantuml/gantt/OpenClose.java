...
import net.sourceforge.plantuml.gantt.ngm.math.PiecewiseConstantTimeWindow;
...
public class OpenClose {
	...
	public OpenClose mutateMe(final OpenClose except) {
		...
		result.workingTimeRanges.addAll(this.workingTimeRanges);
		if (except != null) {
	...
	public PiecewiseConstant asPiecewiseConstant() {
		...
		if (workingTimeRanges.size() > 0)
			result = Combiner.product(result, PiecewiseConstantHours.fromWorkingHours(workingTimeRanges));
