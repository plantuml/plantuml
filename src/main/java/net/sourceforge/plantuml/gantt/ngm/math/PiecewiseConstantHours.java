...
public final class PiecewiseConstantHours extends AbstractPiecewiseConstant {
	 ...
	 *
	 * <p>
	 * The resulting profile is {@link Fraction#ZERO} everywhere by default (no
	 * work), and {@link Fraction#ONE} inside each provided {@link TimeRange}.
	 * Ranges are applied in order; an empty or {@code null} list yields a profile
	 * that is closed for the whole day.
	 * </p>
	 *
	 * @param workingTimeRanges the working hours, for example 8:00-12:00 and
	 *                          13:00-18:00
	...
	public static PiecewiseConstantHours fromWorkingHours(List<TimeRange> workingTimeRanges) {
		...
		for (TimeRange range : workingTimeRanges)
			result = result.with(range, Fraction.ONE);
	...
	public PiecewiseConstantHours with(TimeRange timeRange, Fraction fraction) {

		if (timeRange.getEnd().equals(LocalTime.MIDNIGHT) == false
		...
		result.timeBoundaries.put(timeRange.getStart(), fraction);
		if (timeRange.getEnd().equals(LocalTime.MIDNIGHT) == false)
			result.timeBoundaries.put(timeRange.getEnd(), defaultValue);
		return result;
	}
}
