...
//		}
	...
	// indentation in ASCII yet): a
	// LifeEventTile that is not a destroy is a pure spacer, mirroring
	// EmptyTile -- zero footprint, draws nothing.
	//
	...
	// into still-empty cells, so it is never overwritten.
	// No Unicode variant: like arrowheads, this character doesn't vary
	...
	// livebox rendering at all yet), so the tile itself must draw the X
