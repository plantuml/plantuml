...
public class MonomorphToJson {
	...
	private static JsonArray convertToArray(Monomorph input) {
		...
		for (int i = 0; i < input.size(); i++) {
			final Monomorph element = input.getElementAt(i);
			if (element.getType() == MonomorphType.SCALAR)
				result.add(element.getValue());
			else if (element.getType() == MonomorphType.MAP)
				result.add(convertToObject(element));
			else
	...
	private static JsonObject convertToObject(Monomorph input) {
		...
		for (String key : input.keys()) {
			final Monomorph element = input.getMapValue(key);
			if (element.getType() == MonomorphType.SCALAR)
				result.add(key, element.getValue());
			else if (element.getType() == MonomorphType.MAP)
				result.add(key, convertToObject(element));
			else if (element.getType() == MonomorphType.LIST)
				result.add(key, convertToArray(element));
			else
				throw new UnsupportedOperationException();
