 ...
 *
 * <p>
 * It does <b>not</b> handle diagram chrome (title, header, footer, etc.) nor
 * handwritten mode. The input {@link TextBlock} is expected to be already fully
 * decorated (e.g. by {@link DiagramChromeFactory}).
 */
public class TextBlockExporter {
	...
	private TextBlockExporter(Builder builder) {
		...
		this.stringBounder = builder.fileFormatOption.getDefaultStringBounder(
				builder.skinParam != null ? builder.skinParam : SvgCharSizeHack.NO_HACK);
	}
	...
	private XDimension2D calculateFinalDimension() {
		...
		return new XDimension2D(limitFinder.getMaxX() + 1 + margin.getLeft() + margin.getRight(),
