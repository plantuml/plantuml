...
public class CommandChartAnnotation extends SingleLineCommand2<ChartDiagram> {
	...
	static IRegex getRegexConcat() {
		return RegexConcat.build(CommandChartAnnotation.class.getName(), RegexLeaf.start(), //
				...
				new RegexLeaf("\\("), //
				RegexLeaf.spaceZeroOrMore(), //
				new RegexLeaf(1, "XPOS", "([^,]+)"), //
				RegexLeaf.spaceZeroOrMore(), //
				new RegexLeaf(","), //
				RegexLeaf.spaceZeroOrMore(), //
				new RegexLeaf(1, "YPOS", "([^)]+)"), //
				RegexLeaf.spaceZeroOrMore(), //
				new RegexLeaf("\\)"), //
				RegexLeaf.spaceZeroOrMore(), //
				new RegexOptional(new RegexLeaf(0, "ARROW", "<<arrow>>")), //
				RegexLeaf.end());
	}
	...
	protected CommandExecutionResult executeArg(ChartDiagram diagram, LineLocation location, RegexResult arg,
		...
		try {
			...
			// Not a number, treat as categorical label
			xPosition = xPosStr;
		}

		// Parse Y position - must be numeric
		double yPosition;
		try {
		...
		final boolean showArrow = arrowStr != null;
		final ChartAnnotation annotation = new ChartAnnotation(text, xPosition, yPosition, showArrow);
