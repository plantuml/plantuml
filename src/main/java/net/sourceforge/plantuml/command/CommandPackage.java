...
public class CommandPackage extends SingleLineCommand2<AbstractEntityDiagram> {
	...
	// A bare, unquoted identifier: letters/digits/underscore/dot. Zero-or-more
	// (not one-or-more) so that 'package {' - an anonymous package - still
	// matches branch 1 with an empty CODE1, exactly like the original NAME
	// group ("[^#%s{}]*") allowed an empty match.
	...
	protected String explainArg(LineLocation location, RegexResult arg) {
		...
		if (display == null) {
			...
			sb.append("Starting the package '").append(code).append("' displayed as \"").append(display)
							.append("\"");
		}
