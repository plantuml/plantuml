 ...
 * steps; see {@code net.sourceforge.plantuml.style.LegacyStyleFlattener}.
 *
...
public final class MergedStyleNode {
	...
	/**
	 * Merges every top-level rule in {@code raw} into this node, {@code @media} content
	 * dispatched to its dark half exactly as the plain case is -- what
	 * {@code net.sourceforge.plantuml.style.StyleLoader#parseStyleText} calls to fold a whole
	 * .skin file, or a hand-written {@code <style>...</style>} block, into one throwaway tree,
	 * immediately flattened back to legacy {@code net.sourceforge.plantuml.style.Style} objects
	 * by {@code LegacyStyleFlattener}. {@code counter} must be the very same one across every
	 * call contributing to one logical load (a whole file, or a file followed by its overlays),
	 * exactly like a single top-level {@link #mergeRule} call requires.
	 */
