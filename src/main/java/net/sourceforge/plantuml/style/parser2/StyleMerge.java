 ...
 * combining each property by {@link net.sourceforge.plantuml.style.Specificity} (and, when one
 * match is light-only and another dark-only, folding both into one value) instead of "whichever
 * {@link StyleAtomTrie} happened to visit last" -- the direct counterpart of the legacy
 ...
 * one query per ancestor level, each at its own {@link #mergeAllWithAncestorRank ancestor rank})
 * be assembled by calling this repeatedly and merging the partial results, exactly as
...
public final class StyleMerge {
	 ...
	 * {@link MergeStrategy#KEEP_EXISTING_VALUE_OF_STEREOTYPE} and the accumulated value already
	 * requires at least one stereotype ({@link net.sourceforge.plantuml.style.Specificity#hasStereotype()}),
	 * it is left untouched, whatever {@code incoming}'s own specificity is; otherwise the two are
	 * combined with {@link PrioritizedValue#mergeWith(PrioritizedValue)}.
	 */
	public static Map<PName, PrioritizedValue> mergeInto(Map<PName, PrioritizedValue> accumulated,
		...
		for (Map.Entry<PName, PrioritizedValue> ent : incoming.entrySet()) {
			...
			if (strategy == MergeStrategy.KEEP_EXISTING_VALUE_OF_STEREOTYPE && existing != null
					&& existing.getSpecificity().hasStereotype())
				continue;
	...
	/**
	 * Folds every match in {@code matches}, in order, into one property map by specificity -- the
	...
	/**
	 * Same as {@link #mergeAll}, but first applies {@link PrioritizedValue#withAncestorRank(int)}
	 * to the properties of every match whose declaration is starred -- exactly
	 * {@code StyleBuilder#getMergedStyleSpecial}'s
	 * {@code if (key.isStarred()) tmp = tmp.withAncestorRank(ancestorRank);}. This is what an
	 ...
	 * first place, and giving each one its own ancestor-cascade tier (see
	 * {@link net.sourceforge.plantuml.style.Specificity#withAncestorRank(int)}) is what lets a
	 * closer ancestor's catch-all still lose to the element's own explicit declaration, or win
	 * over a farther ancestor's, once the results of several such calls (one per ancestor level,
	 * each at its own rank) are merged together with {@link #mergeInto}.
	 */
	public static Map<PName, PrioritizedValue> mergeAllWithAncestorRank(List<CompiledStyleRule> matches,
			int ancestorRank, MergeStrategy strategy) {
		Map<PName, PrioritizedValue> result = new EnumMap<PName, PrioritizedValue>(PName.class);
		for (CompiledStyleRule rule : matches) {
			Map<PName, PrioritizedValue> values = rule.getPrioritizedProperties();
			if (rule.getLevelConstraint().isStar())
				values = withAncestorRank(values, ancestorRank);
			result = mergeInto(result, values, strategy);
		}
		return result;
	}

	/** Applies {@link PrioritizedValue#withAncestorRank(int)} to every value, keeping the same properties. */
	private static Map<PName, PrioritizedValue> withAncestorRank(Map<PName, PrioritizedValue> values, int rank) {
		...
		for (Map.Entry<PName, PrioritizedValue> ent : values.entrySet())
			result.put(ent.getKey(), ent.getValue().withAncestorRank(rank));
		return result;
	}
