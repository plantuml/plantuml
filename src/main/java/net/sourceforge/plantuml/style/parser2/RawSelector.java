...
public final class RawSelector {
	...
	/**
	 * A normalized key identifying this selector among siblings of the same {@link Kind}, so
	 * that two occurrences written slightly differently (e.g. {@code .Foo} and {@code .foo})
	 * still merge into the same node. Meaningless -- and unused -- for {@link Kind#NAME},
	 * which merges through the {@link SName} enum itself instead.
	 */
	public String canonicalOtherKey() {
		switch (kind) {
		case STEREOTYPE:
			return raw.substring(1).toLowerCase();
		case DEPTH:
			return "depth(" + depth + ")";
		case UNKNOWN:
			return raw.toLowerCase();
		default:
			throw new IllegalStateException("Not applicable to " + kind);
		}
	}
