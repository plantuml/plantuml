...
public class ValueImpl implements Value {
	...
	public static ValueImpl dark(String value, AutomaticCounter counter) {
		return new ValueImpl(new DarkString(null, Objects.requireNonNull(value), Specificity.atOrder(counter.getNextInt())));
	}

	public static ValueImpl regular(String value, AutomaticCounter counter) {
		return new ValueImpl(new DarkString(Objects.requireNonNull(value), null, Specificity.atOrder(counter.getNextInt())));
	}

	public static ValueImpl regular(String value, Specificity specificity) {
		return new ValueImpl(new DarkString(Objects.requireNonNull(value), null, specificity));
	}

	/**
	 * A value that may already carry both a light and a dark half at an explicit specificity --
	 ...
	 * null.
	 */
	public static ValueImpl of(String light, String dark, Specificity specificity) {
		return new ValueImpl(new DarkString(light, dark, specificity));
	}
	...
	public Value mergeWith(Value other) {
		...
		if (other instanceof ValueColor) {
			if (other.getSpecificity().isBiggerThan(getSpecificity()))
				return other;
			return this;
		}
	...
	public Value withAncestorRank(int rank) {
		return new ValueImpl(value.withAncestorRank(rank));
	}

	public Value withStereotypeCount(int count) {
		return new ValueImpl(value.withStereotypeCount(count));
	}
