...
public abstract class AbstractUGraphic<O> extends AbstractCommonUGraphic {
	...
	// A flat array indexed by UShapeKind, not a Map<Class<?>, ...>: a hash of the
	// shape's Class
	// on every draw bought nothing, since only a dozen shape classes are ever
	// registered and
	// none of them has a subclass. It would have been nice to write
	// Map<Class<SHAPE>,
	// UDriver<SHAPE, O>> and let the type system relate the two sides, which Java
	// cannot express
	// -- see
	// https://stackoverflow.com/questions/416540/java-map-with-values-limited-by-keys-type-parameter
	// registerDriver still takes the Class, so each call site is still checked that
	// way; only
	// the storage underneath changed.
	//
	// Null until a driver is actually registered, which only ever happens on the
	// UGraphic a copy
	// chain starts from: a copy takes the table from the instance it copies (see
	// copy(AbstractUGraphic)), so filling this in as a field initializer would mean
	// one throwaway
	// array per apply() -- some fifteen thousand of them on a 200-message sequence
	// diagram.
	private UDriver<?, O>[] drivers;

	// Set once this UGraphic took its driver table from another one rather than
	// filling its own.
	// A copy shares the table -- that is the whole point of copy(AbstractUGraphic)
	// -- so
	// registering into it afterwards would overwrite entries the original and every
	// sibling copy
	// are still using, and every driver registered on a copy would leak that copy's
	// state to all
	// of them. No backend should do it; this makes the mistake loud rather than
	// silent.
	...
	/**
	 * Makes this UGraphic a copy of {@code other}, sharing its driver table rather
	 * than building one of its own -- which is only sound because no driver
	 * captures anything that differs between copies: the clip they used to hold the
	 * UGraphic for now reaches them through {@code UParam#getClip()}, and what is
	 * left (a StringBounder, a dpi factor, a FileFormat, an EpsStrategy) is fixed
	 * for the whole family. A copy must therefore not call its own register();
	 * registerDriver refuses to run after this point.
	 */
	...
	/**
	 * Adopts a driver table built elsewhere, instead of filling one through
	 * registerDriver: a backend whose drivers are all stateless can hold a single
	 * static table and hand it to every UGraphic it creates, rather than building
	 * one per copy chain. The table is then shared by every instance of that
	 * backend for the life of the process, so it must stay immutable --
	 * registerDriver refuses to run after this, exactly as it does after
	 * copy(AbstractUGraphic).
	 */
	protected final void useDrivers(UDriver<?, O>[] shared) {
		this.drivers = shared;
		this.driversShared = true;
	}

	/**
	 * The do-nothing driver of {@link #ignoreShape}, for a backend filling a table
	 * statically.
	 */
	@SuppressWarnings("unchecked")
	protected static <SHAPE extends UShape, O> UDriver<SHAPE, O> noopDriver() {
		return (UDriver<SHAPE, O>) NOOP_DRIVER;
	}
	...
	public final <SHAPE extends UShape> void draw(SHAPE shape) {
		...
		// UShapeKind.UNKNOWN is what every shape no driver handles reports, and nothing
		// is ever
