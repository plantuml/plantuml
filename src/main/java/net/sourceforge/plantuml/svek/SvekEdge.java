...
public class SvekEdge extends XAbstractEdge implements XEdge, UDrawable {
	...
	public void appendLine(GraphvizVersion graphvizVersion, StringBuilder sb, DotMode dotMode, DotSplines dotSplines) {
		...
		// sb.append(startUid);
		// } else {
		String dotStartId = startUid.getFullString();
		String dotEndId = endUid.getFullString();
		...
		if (bothClusters == false && startInsideHead == false && lhead != null) {
			...
			if (childNodes.size() >= 2 && childNodes.size() <= 3) {
				...
				for (int i = 0; i < childNodes.size(); i++) {
					if (i != mainIdx) {
						sb.append(startUid.getFullString() + "->" + childNodes.get(i).getUid()
								+ "[style=invis,lhead=" + lhead.getClusterId() + "];");
						SvekUtils.println(sb);
					}
				}
			}
		...
		if (bothClusters == false && endInsideTail == false && ltail != null) {
			...
			if (childNodes.size() >= 2 && childNodes.size() <= 3) {
				...
				for (int i = 0; i < childNodes.size(); i++) {
					if (i != mainIdx) {
						sb.append(childNodes.get(i).getUid() + "->" + endUid.getFullString()
								+ "[style=invis,ltail=" + ltail.getClusterId() + "];");
						SvekUtils.println(sb);
					}
				}
			}
	...
	private static boolean isNodeInsideCluster(String nodeUid, Cluster cluster) {
	...
	private static boolean isNodeInCluster(SvekNode node, Cluster cluster) {
		if (node == null)
			return false;
		for (SvekNode child : cluster.getNodes())
			if (child == node)
