...
public class CucaGroupWrapper implements ICucaGroupWrapper {

	IGroup group;

	protected CucaGroupWrapper(IGroup group) {
		this.group = group;
	}
	...
	@Override
	public List<ICucaGroupWrapper> getChildren(){
		return group.getChildren().stream()
						.map(group -> Factory.getInstance().createGroupWrapper(group))
						.collect(Collectors.toList());
	}

	@Override
	public List<ICucaLeafWrapper> getLeafs(){
		return group.getLeafsDirect().stream()
						.map(leaf -> Factory.getInstance().createLeafWrapper(leaf))
						.collect(Collectors.toList());
	}

	@Override
	public void acceptVisitor(ICucaDiagramVisitor visitor) {
		visitor.visitCucaGroup(this);
	}
}
