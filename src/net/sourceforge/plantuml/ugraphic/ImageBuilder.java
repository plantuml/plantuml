...
public class ImageBuilder {
    ...
    public ImageData writeImageTOBEMOVED(List<ComponentDisplayInfo> displayComponent, FileFormatOption fileFormatOption,
        ...
        return writeImageTOBEMOVED(displayComponent, fileFormatOption, os, affineTransformations);
    }

    private ImageData writeImageTOBEMOVED(FileFormatOption fileFormatOption, OutputStream os,
            Animation affineTransforms) throws IOException {
        final LimitFinder limitFinder = new LimitFinder(TextBlockUtils.getDummyStringBounder(), true);
        udrawable.drawU(limitFinder);
        Dimension2D dim = new Dimension2DDouble(limitFinder.getMaxX() + 1 + margin1 + margin2,
    ...
    private ImageData writeImageTOBEMOVED(List<ComponentDisplayInfo> displayComponent,
            FileFormatOption fileFormatOption, OutputStream os, Animation affineTransforms) throws IOException {
        final LimitFinder limitFinder = new LimitFinder(TextBlockUtils.getDummyStringBounder(), true);
        udrawable.drawU(limitFinder);
        Dimension2D dim = new Dimension2DDouble(limitFinder.getMaxX() + 1 + margin1 + margin2,
                limitFinder.getMaxY() + 1 + margin1 + margin2);
        ...
        final UGraphic2 ug = createUGraphic(displayComponent, fileFormatOption, dim, affineTransforms, dx, dy);
        udrawable.drawU(handwritten(ug.apply(new UTranslate(margin1, margin1))));
        ug.flushUg();
        ug.writeImageTOBEMOVED(os, metadata, 96);
        os.flush();
    ...
    private UGraphic2 createUGraphic(List<ComponentDisplayInfo> displayComponents, FileFormatOption fileFormatOption,
            final Dimension2D dim, Animation affineTransforms, double dx, double dy) {
        final FileFormat fileFormat = fileFormatOption.getFileFormat();
        switch (fileFormat) {
        case PNG:
            return createUGraphicPNG(colorMapper, dpiFactor, dim, mybackcolor, affineTransforms, dx, dy);
        case SVG:
            return createUGraphicSVG(displayComponents, colorMapper, dpiFactor, dim, mybackcolor,
    ...
    private UGraphic2 createUGraphicSVG(List<ComponentDisplayInfo> displayComponents, ColorMapper colorMapper,
            double scale, Dimension2D dim, HtmlColor mybackcolor, String svgLinkTarget) {
        Color backColor = Color.WHITE;
        if (mybackcolor instanceof HtmlColorSimple) {
            backColor = colorMapper.getMappedColor(mybackcolor);
        }
        ...
        if (mybackcolor instanceof HtmlColorGradient) {
            ...
            ug = new UGraphicSvg(displayComponents, colorMapper, StringUtils.getAsHtml(backColor), false, scale,
                    svgLinkTarget);
        }
        return ug;
    }
    ...
    private UGraphic2 createUGraphicPNG(ColorMapper colorMapper, double dpiFactor, final Dimension2D dim,
        ...
        /*
         * if (rotation) { builder = new EmptyImageBuilder((int)
         * (dim.getHeight() * dpiFactor), (int) (dim.getWidth() * dpiFactor),
         * backColor); graphics2D = builder.getGraphics2D();
