...
public class CucaDiagramFileMakerHectorB2 implements CucaDiagramFileMaker {
    ...
    public ImageData createFile(OutputStream os, List<String> dotStrings, FileFormatOption fileFormatOption)
        ...
        for (Pin pin : skeleton.getPins()) {
            minMax = minMax.addPoint(
                    unlinarCompressedPlan.uncompress(getX(pin), getY(pin), UnlinearCompression.Rounding.BORDER_2));
        }
    ...
    private void drawPin(UGraphic ug, Pin pin) {
        ...
        ug.apply(new UChangeColor(HtmlColorUtils.BLACK)).apply(new UChangeBackColor(HtmlColorUtils.BLACK))
                .apply(new UTranslate(x, y)).draw(rect);
    }
    ...
    private void drawPinLink(UGraphic ug, PinLink pinLink) {
        ...
        // final Point2D p1 = unlinarCompressedPlan.uncompress(pp1,
        // UnlinearCompression.Rounding.CENTRAL);
        // final Point2D p2 = unlinarCompressedPlan.uncompress(pp2,
        // UnlinearCompression.Rounding.CENTRAL);
        // final SmartConnection connection = new SmartConnection(p1, p2, new
        // ArrayList<Box2D>());
