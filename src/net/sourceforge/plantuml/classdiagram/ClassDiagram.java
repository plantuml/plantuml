...
public class ClassDiagram extends AbstractClassOrObjectDiagram {
    ...
    private ILeaf createEntityWithNamespace(Code fullyCode, Display display, LeafType type, USymbol symbol) {
        ...
        return createLeafInternal(fullyCode, Display.isNull(display)
                ? Display.getWithNewlines(fullyCode.getShortName(getLeafs())).withCreoleMode(CreoleMode.SIMPLE_LINE)
                : display, type, group, symbol);
    }
    ...
    final protected ImageData exportDiagramInternal(List<ComponentDisplayInfo> displayComponents, OutputStream os,
            int index, FileFormatOption fileFormatOption) throws IOException {
        if (useLayoutExplicit != 0) {
            return exportLayoutExplicit(os, index, fileFormatOption);
        }
        return super.exportDiagramInternal(displayComponents, os, index, fileFormatOption);
    }
