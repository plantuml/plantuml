 ...
 *
 * PlantUML is free software; you can redistribute it and/or modify it under the terms of the GNU
 * General Public License as published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * PlantUML distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even
...
public final class CucaDiagramFileMakerSvek2 {
  ...
  public CucaDiagramFileMakerSvek2(DotData dotData, EntityFactory entityFactory, UmlSource source,
      Pragma pragma) {
  ...
  public IEntityImage createFileForConcurrentState() {
    return new CucaDiagramFileMakerSvek2InternalImage(dotData.getLeafs(),
        dotData.getTopParent().getConcurrentSeparator(), dotData.getSkinParam(),
        dotData.getSkinParam().getBackgroundColor());
  ...
  public IEntityImage createFile(String... dotStrings) {
    ...
    for (Link link : dotData.getLinks()) {
      ...
      try {
        ...
        if (shapeUid1.startsWith(Cluster.CENTER_ID)) {
          ...
          ltail = getCluster2(link.getEntity1());
        }
        Cluster lhead = null;
        if (shapeUid2.startsWith(Cluster.CENTER_ID)) {
          // final Group g2 = ((IEntityMutable)
          // link.getEntity2()).getContainerOrEquivalent();
          lhead = getCluster2(link.getEntity2());
        }
        final ISkinParam skinParam = dotData.getSkinParam();
        final FontConfiguration labelFont =
            new FontConfiguration(skinParam, FontParam.GENERIC_ARROW, null);

        final Line line = new Line(shapeUid1, shapeUid2, link, colorSequence, ltail, lhead,
            skinParam, stringBounder, labelFont, getBibliotekon(),
            dotStringFactory.getGraphvizVersion(), dotData.getPragma());

        getBibliotekon().addLine(line);

        if (link.getEntity1().isGroup() == false
            && link.getEntity1().getEntityType() == LeafType.NOTE
        ...
        } else if (link.getEntity2().isGroup() == false
    ...
    final boolean trace =
        OptionFlags.getInstance().isKeepTmpFiles() || OptionFlags.TRACE_DOT || isSvekTrace();
    final String svg;
    try {
  ...
  private void printEntity(ILeaf ent) {
    ...
    final Shape shape = new Shape(image, image.getShapeType(), dim.getWidth(), dim.getHeight(),
        colorSequence, ent.isTop(), image.getShield(), ent.getEntityPosition());
  ...
  private IEntityImage printEntityInternal(ILeaf ent) {
    ...
    if (ent.getSvekImage() == null) {
      ...
      return createEntityImageBlock(ent, skinParam, dotData.isHideEmptyDescriptionForState(),
          dotData, getBibliotekon(), dotStringFactory.getGraphvizVersion(),
  ...
  public static IEntityImage createEntityImageBlock(ILeaf leaf, ISkinParam skinParam,
    ...
    if (leaf.getEntityType().isLikeClass()) {
      final EntityImageClass entityImageClass =
          new EntityImageClass(graphvizVersion, leaf, skinParam, portionShower);
    ...
    if (leaf.getEntityType() == LeafType.STATE) {
      ...
      if (leaf.getStereotype() != null
          && "<<sdlreceive>>".equals(leaf.getStereotype().getLabel(false))) {
    ...
    if (leaf.getEntityType() == LeafType.SYNCHRO_BAR
        || leaf.getEntityType() == LeafType.STATE_FORK_JOIN) {
    ...
    if (leaf.getEntityType() == LeafType.EMPTY_PACKAGE) {
      if (leaf.getUSymbol() != null) {
        return new EntityImageDescription(leaf,
            new SkinParamForecolored(skinParam, HtmlColorUtils.BLACK), portionShower);
      }
    ...
    for (IGroup g : dotData.getGroupHierarchy().getChildrenGroups(parent)) {
      ...
      if (dotData.isEmpty(g) && g.getGroupType() == GroupType.PACKAGE) {
        ...
        if (g.getColors(dotData.getSkinParam()).getColor(ColorType.BACK) == null) {
          final ColorParam param =
              symbol == null ? ColorParam.packageBackground : symbol.getColorParamBack();
  ...
  private void printGroup(IGroup g) {
    ...
    final TextBlock stereoAndTitle =
        TextBlockUtils.mergeTB(stereo, title, HorizontalAlignment.CENTER);
    ...
    if (dimLabel.getWidth() > 0) {
      ...
      if (members.size() == 0) {
        ...
        attribute =
            new MethodsOrFieldsArea(members, FontParam.STATE_ATTRIBUTE, dotData.getSkinParam());
      }
      ...
      titleAndAttributeWidth =
          (int) Math.max(dimLabel.getWidth(), attributeWidth) + suppWidthBecauseOfShape;
  ...
  private TextBlock getStereoBlock(IGroup g) {
    ...
    if (stereotype.getSprite() != null) {
      return dotData.getSkinParam().getSprite(stereotype.getSprite())
          .asTextBlock(stereotype.getHtmlColor());
    }
    ...
    return Display.create(stereos).create(
        new FontConfiguration(dotData.getSkinParam(), fontParam, stereotype),
