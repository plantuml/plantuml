...
public class Histogram implements TimeDrawing {
	...
	private List<String> getStatesAt(TimeTick tick) {
		...
		for (int i = 0; i < changes.size(); i++) {
			int tickWithCurrentChangeTimeComparisonResult = changes.get(i).getWhen().compareTo(tick);
			...
			if (tickWithCurrentChangeTimeComparisonResult > 0) {
				int changeIndex;
			    if (i == 0) {
			        // if this time tick was not yet defined in any place, and is less then the first one, assume it's
					// the leftmost
			    	changeIndex = 0;
				}
				else {
