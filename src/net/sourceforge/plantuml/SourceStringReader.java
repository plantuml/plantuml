...
public class SourceStringReader {
    ...
    public String generateImage(List<ComponentDisplayInfo> displayComponents, OutputStream os,
            FileFormatOption fileFormatOption) throws IOException {
        return generateImage(displayComponents, os, 0, fileFormatOption);
    }
    ...
    public String generateImage(List<ComponentDisplayInfo> displayComponents, OutputStream os, int numImage,
            FileFormatOption fileFormatOption) throws IOException {
        if (blocks.size() == 0) {
            noStartumlFound(os, fileFormatOption);
            return null;
        ...
        for (BlockUml b : blocks) {
            final UmlDiagram system = (UmlDiagram) b.getDiagram();
            final int nbInSystem = system.getNbImages();
            if (numImage < nbInSystem) {
                // final CMapData cmap = new CMapData();
                final ImageData imageData = system.exportDiagram(displayComponents, os, numImage, fileFormatOption);
                if (imageData.containsCMapData()) {
                    return system.getDescription().getDescription() + "\n" + imageData.getCMapData("plantuml");
                }
                return system.getDescription().getDescription();
    ...
    public DiagramDescription generateDiagramDescription(OutputStream os, int numImage,
        ...
        for (BlockUml b : blocks) {
            ...
            if (numImage < nbInSystem) {
                ...
                final ImageData imageData = system.exportDiagram(os, numImage, fileFormatOption);
                if (imageData.containsCMapData()) {
                    return ((DiagramDescriptionImpl) system.getDescription())
                            .withCMapData(imageData.getCMapData("plantuml"));
                }
