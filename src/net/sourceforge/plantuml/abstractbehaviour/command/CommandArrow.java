...
public class CommandArrow extends SingleLineCommand2<AbstractBehaviourDiagram> {
	...
	private Participant getOrCreateParticipant(AbstractBehaviourDiagram system, RegexResult arg2, String n) {
		final String code;
	...
	protected CommandExecutionResult executeArg(AbstractBehaviourDiagram diagram, RegexResult arg) {
		...
		if (p1.getType() != ParticipantType.PARTICIPANT)
			return CommandExecutionResult.ok();
		if (p2.getType() != ParticipantType.PARTICIPANT)
			return CommandExecutionResult.ok();
		...
		if (arg.get("MESSAGE", 0) == null) {
			...
			if (message.contains("!")) {
				String controller = message.substring(0, message.indexOf("!"));
				message = message.substring(message.indexOf("!")+1);
				if (controller.equalsIgnoreCase(p2.getCode())) {
					Participant p = p2;
					p2 = p1;
					p1 = p;
				}
